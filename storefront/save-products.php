...
$mode = isset($data['mode']) ? $data['mode'] : 'append';

if ($mode === 'replace') {
    // Direct replace mode for Admin management deletions/edits
    $finalProducts = $data['products'];
} else {
    // Append mode (Default) with Smart Merge
    $existingProducts = [];
...
// Fast Image Downloader (1s strict timeout per image)
...
echo json_encode([
    "success" => true, 
    "message" => "Successfully updated " . count($finalProducts) . " total products!",
